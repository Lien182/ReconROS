...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreadb2_img_output_hw);
	uint64_t mbox_value;
	...
	while(1) {
		...
		mbox_value = MBOX_GET(rthreadb2_start_mbox);
		...
		ROS_PUBLISH(rthreadb2_pubdata2, rthreadb2_img_output_hw);
		}

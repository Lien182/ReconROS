...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreadb3_img_output_hw);
	uint64_t mbox_value;
	...
	while(1) {
		...
		mbox_value = MBOX_GET(rthreadb3_start_mbox);
		...
		ROS_PUBLISH(rthreadb3_pubdata2, rthreadb3_img_output_hw);
		}

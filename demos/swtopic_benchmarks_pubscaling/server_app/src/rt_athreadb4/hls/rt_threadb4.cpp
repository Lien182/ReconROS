...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreadb4_img_output_hw);
	uint64_t mbox_value;
	...
	while(1) {
		...
		mbox_value = MBOX_GET(rthreadb4_start_mbox);
		...
		ROS_PUBLISH(rthreadb4_pubdata2, rthreadb4_img_output_hw);
		}

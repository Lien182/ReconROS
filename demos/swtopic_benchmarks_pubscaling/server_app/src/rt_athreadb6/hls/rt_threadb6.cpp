...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreadb6_img_output_hw);
	uint64_t mbox_value;
	...
	while(1) {
		...
		mbox_value = MBOX_GET(rthreadb6_start_mbox);
		...
		ROS_PUBLISH(rthreadb6_pubdata2, rthreadb6_img_output_hw);
		}

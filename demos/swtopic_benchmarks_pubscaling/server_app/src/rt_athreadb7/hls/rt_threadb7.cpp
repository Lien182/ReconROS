...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreadb7_img_output_hw);
	uint64_t mbox_value;
	...
	while(1) {
		...
		mbox_value = MBOX_GET(rthreadb7_start_mbox);
		...
		ROS_PUBLISH(rthreadb7_pubdata2, rthreadb7_img_output_hw);
		}

...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreada5_img_output);
	uint64_t msg;
	...
	while(1) {
		...
		msg = ROS_SUBSCRIBE_TAKE(rthreada5_subdata2, rthreada5_img_input_hw);
		...
		MBOX_PUT(rthreada5_finish_mbox, mbox_value);
		...
		ap_wait();							
		ROS_PUBLISH(rthreada5_pubdata, rthreada5_img_output);
		}
	}
}

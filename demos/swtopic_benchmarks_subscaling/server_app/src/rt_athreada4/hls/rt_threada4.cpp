...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreada4_img_output);
	uint64_t msg;
	...
	while(1) {
		...
		msg = ROS_SUBSCRIBE_TAKE(rthreada4_subdata2, rthreada4_img_input_hw);
		...
		MBOX_PUT(rthreada4_finish_mbox, mbox_value);
		...
		ap_wait();							
		ROS_PUBLISH(rthreada4_pubdata, rthreada4_img_output);

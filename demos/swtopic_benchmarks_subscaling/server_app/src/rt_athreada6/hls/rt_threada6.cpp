...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreada6_img_output);
	uint64_t msg;
	...
	while(1) {
		...
		msg = ROS_SUBSCRIBE_TAKE(rthreada6_subdata2, rthreada6_img_input_hw);
		...
		MBOX_PUT(rthreada6_finish_mbox, mbox_value);
		...
		ap_wait();							
		ROS_PUBLISH(rthreada6_pubdata, rthreada6_img_output);
		}
	}
}

...
THREAD_ENTRY() {
	...
	output_buffer_addr = MEMORY_GETOBJECTADDR(rthreada2_img_output);
	uint64_t msg;
	...
	while(1) {
		...
		msg = ROS_SUBSCRIBE_TAKE(rthreada2_subdata2, rthreada2_img_input_hw);
		...
		MBOX_PUT(rthreada2_finish_mbox, mbox_value);
		...
		ap_wait();							
		ROS_PUBLISH(rthreada2_pubdata, rthreada2_img_output);

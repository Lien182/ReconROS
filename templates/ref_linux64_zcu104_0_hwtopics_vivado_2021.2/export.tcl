...
proc reconos_hw_setup {new_project_name new_project_path reconos_ip_dir} {
    ...
    ##############################################################
      ...
      <<generate for HWTOPICS(Type == "ONE_TO_ONE")>>

        #Connect broadcaster with subscribers of hwtopic
        <<=generate for SUBSCRIBERS(FIFOSIZE == "0")=>>
          connect_bd_intf_net [get_bd_intf_pins slot_<<SlotId>>/<<SlotPortName>>] [get_bd_intf_pins slot_<<Pub_SlotId>>/<<Pub_SlotPortName>>]
        <<=end generate=>>

        <<=generate for SUBSCRIBERS(FIFOSIZE != "0")=>>
          create_bd_cell -type ip -vlnv xilinx.com:ip:axis_data_fifo:2.0 axis_data_fifo_<<FIFOID>>
          connect_bd_intf_net [get_bd_intf_pins slot_<<SlotId>>/<<SlotPortName>>] [get_bd_intf_pins axis_data_fifo_<<FIFOID>>/M_AXIS]
          connect_bd_intf_net [get_bd_intf_pins slot_<<Pub_SlotId>>/<<Pub_SlotPortName>>] [get_bd_intf_pins axis_data_fifo_<<FIFOID>>/S_AXIS]
          connect_bd_net [get_bd_pins axis_data_fifo_<<FIFOID>>/s_axis_aresetn] [get_bd_pins reset_0/interconnect_aresetn]
          connect_bd_net [get_bd_pins axis_data_fifo_<<FIFOID>>/s_axis_aclk] [get_bd_pins reconos_clock_0/CLK0_Out]

          set_property -dict [list CONFIG.FIFO_DEPTH {<<FIFOSIZE>>} CONFIG.FIFO_MEMORY_TYPE {distributed}] [get_bd_cells axis_data_fifo_<<FIFOID>>]
        <<=end generate=>>
    ...
    ##############################################################
    <<generate for HWTOPICS(Type == "ONE_TO_N")>>
    ...
    # AXIS-Broadcaster-based architecture for hardware topics
      ...
      # Broadcaster
        ...
        connect_bd_net [get_bd_pins axis_broadcaster_<<Name>>/aclk] [get_bd_pins reconos_clock_0/CLK<<SYSCLK>>_Out]
	      connect_bd_net [get_bd_pins axis_broadcaster_<<Name>>/aresetn] [get_bd_pins reset_0/interconnect_aresetn]
        
        # Connect broadcaster with subscribers of hwtopic
        <<=generate for SUBSCRIBERS(FIFOSIZE == "0")=>>
          connect_bd_intf_net [get_bd_intf_pins axis_broadcaster_<<Name>>/M0<<SubNr>>_AXIS] [get_bd_intf_pins slot_<<SlotId>>/<<SlotPortName>>]
        <<=end generate=>>

        <<=generate for SUBSCRIBERS(FIFOSIZE != "0")=>>
          create_bd_cell -type ip -vlnv xilinx.com:ip:axis_data_fifo:2.0 axis_data_fifo_<<FIFOID>>
          connect_bd_intf_net [get_bd_intf_pins slot_<<SlotId>>/<<SlotPortName>>] [get_bd_intf_pins axis_data_fifo_<<FIFOID>>/M_AXIS]
          connect_bd_intf_net [get_bd_intf_pins  axis_broadcaster_<<Name>>/M0<<SubNr>>_AXIS] [get_bd_intf_pins axis_data_fifo_<<FIFOID>>/S_AXIS]
          connect_bd_net [get_bd_pins axis_data_fifo_<<FIFOID>>/s_axis_aresetn] [get_bd_pins reset_0/interconnect_aresetn]
          connect_bd_net [get_bd_pins axis_data_fifo_<<FIFOID>>/s_axis_aclk] [get_bd_pins reconos_clock_0/CLK0_Out]
      ...
      connect_bd_intf_net [get_bd_intf_pins slot_<<SlotId>>/<<SlotPortName>>] [get_bd_intf_pins axis_broadcaster_<<Name>>/S_AXIS]
      <<=end generate=>>
    <<end generate>>
    ...
    <<generate for HWTOPICS(Type == "N_TO_ONE")>>
    startgroup
        ...
        connect_bd_net [get_bd_pins axis_interconnect_<<Name>>/ACLK] [get_bd_pins reconos_clock_0/CLK<<SYSCLK>>_Out]
        connect_bd_net [get_bd_pins axis_interconnect_<<Name>>/ARESETN] [get_bd_pins reset_0/interconnect_aresetn]
        connect_bd_net [get_bd_pins axis_interconnect_<<Name>>/M00_AXIS_ACLK] [get_bd_pins reconos_clock_0/CLK<<SYSCLK>>_Out]
        ...
        <<=generate for PUBLISHERS=>>
            connect_bd_intf_net -boundary_type upper [get_bd_intf_pins axis_interconnect_<<Name>>/S0<<PubNr>>_AXIS] [get_bd_intf_pins slot_<<SlotId>>/<<SlotPortName>>]
            connect_bd_net [get_bd_pins axis_interconnect_<<Name>>/S0<<PubNr>>_AXIS_ACLK] [get_bd_pins reconos_clock_0/CLK<<SYSCLK>>_Out]
            connect_bd_net [get_bd_pins axis_interconnect_<<Name>>/S0<<PubNr>>_AXIS_ARESETN] [get_bd_pins reset_0/interconnect_aresetn]
        <<=end generate=>>

        # Connect broadcaster with subscribers of hwtopic
        <<=generate for SUBSCRIBERS(FIFOSIZE == "0")=>>
        connect_bd_intf_net [get_bd_intf_pins slot_<<SlotId>>/<<SlotPortName>>] -boundary_type upper [get_bd_intf_pins axis_interconnect_<<Name>>/M00_AXIS]
        <<=end generate=>>

        <<=generate for SUBSCRIBERS(FIFOSIZE != "0")=>>
          create_bd_cell -type ip -vlnv xilinx.com:ip:axis_data_fifo:2.0 axis_data_fifo_<<FIFOID>>
          connect_bd_intf_net [get_bd_intf_pins slot_<<SlotId>>/<<SlotPortName>>] [get_bd_intf_pins axis_data_fifo_<<FIFOID>>/M_AXIS]
          connect_bd_intf_net [get_bd_intf_pins axis_interconnect_<<Name>>/M00_AXIS] [get_bd_intf_pins axis_data_fifo_<<FIFOID>>/S_AXIS]
          connect_bd_net [get_bd_pins axis_data_fifo_<<FIFOID>>/s_axis_aresetn] [get_bd_pins reset_0/interconnect_aresetn]
          connect_bd_net [get_bd_pins axis_data_fifo_<<FIFOID>>/s_axis_aclk] [get_bd_pins reconos_clock_0/CLK0_Out]
    ...
    # AXIS-Interconnect-Broadcaster-based architecture for hardware topics
        ...
        <<generate for HWTOPICS(Type == "N_TO_M")>>
        startgroup
        create_bd_cell -type ip -vlnv xilinx.com:ip:axis_interconnect:2.1 axis_interconnect_<<Name>>
        endgroup
        # Number of slave interfaces (publisher), number of master interfaces (subscriber)
        ...
        # Connect axis interconnect output (master) with axis broadcaster input (slave)
        connect_bd_intf_net -boundary_type upper [get_bd_intf_pins axis_interconnect_<<Name>>/M00_AXIS] [get_bd_intf_pins axis_broadcaster_<<Name>>/S_AXIS]
    ...
    #delete_bd_objs [get_bd_intf_nets axis_broadcaster_nicehwtopic_M08_AXIS]
    #create_bd_cell -type ip -vlnv xilinx.com:ip:axis_data_fifo:2.0 axis_data_fifo_0
    #connect_bd_intf_net [get_bd_intf_pins axis_broadcaster_nicehwtopic/M08_AXIS] [get_bd_intf_pins axis_data_fifo_0/S_AXIS]
    #connect_bd_intf_net [get_bd_intf_pins axis_data_fifo_0/M_AXIS] [get_bd_intf_pins slot_9/nicehwtopic_in]
    #connect_bd_net [get_bd_pins axis_data_fifo_0/s_axis_aresetn] [get_bd_pins reset_0/interconnect_aresetn]
    #connect_bd_net [get_bd_pins axis_data_fifo_0/s_axis_aclk] [get_bd_pins reconos_clock_0/CLK0_Out]

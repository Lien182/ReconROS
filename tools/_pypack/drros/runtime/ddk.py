...
import drros.runtime.project as project
import drros.utils.shutil2 as shutil2
...
logging.basicConfig(format="[drros-toolchain | %(name)s] %(levelname)s: %(message)s")
...
def main():
	...
	parser = argparse.ArgumentParser(prog="ddk", description="""
	  Distributed ReconROS Development Kit- Toolchain integrating different
	...
	if args.cmd == None:
		...
		while(True):
			try:
				if args.log == "debug":
					i = input("DRROS Toolchain [" + prjfile + "]> ")
				else:
					i = input("DRROS Toolchain> ")

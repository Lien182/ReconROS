    ...
    for key in msg.keys():
        if(msg[key][-1]=="]"):
            ...
            if((m is None) or (len(m) == 0)):
                log.debug("No array size given in msg definition for {}. Inferring 1 as array size".format(msg[key]))
                log.debug(msg[key])
                m = 1
                ...
                log.debug(m)
                m = int(m[0])
                log.debug("Found array of size " + str(m))
            temp = re.sub("[\[].*?[\]]", "", msg[key])
            if(temp in msg_lib.keys()):
                ...
                log.debug("No match found in msg_lib for key {}".format(msg[key]))
...
def _parse_array_size(string):
    ...
    if((m is None) or (len(m) == 0)):
        log.debug("No array size given in msg definition for {}.".format(string))
        m = -1
...
def _parse_dwidth(string):
    ...
    if(string.startswith("string")):
        ...
        try:
            ...
            log.debug("Data width could not be determined for array of type " + string)
            width = 0
...
def _sort_into_datatypes(msg, primitive_lib):
    ...
    for key in msg.keys():
        if(msg[key][-1]=="]"): # array 
            ...
            array_dict["dtype"] = primitive_lib[re.sub("[\[].*?[\]]", "", msg[key])]+"*"
            array_dict["dwidth"] = _parse_dwidth(array_dict["dtype"])
            if(m > 0):
                array_dict["num_elems"] = m
            else:
                array_dict["num_elems"] = (key + "_array_length").replace(".", "_").upper();
            ...
            array_dict["size"] = primitive_lib["size_t"]
            array_dict["capacity"] = primitive_lib["size_t"]
            array_dict["dtype"] = "char*"
            array_dict["dwidth"] = _parse_dwidth("string")
            if(m > 0):
                array_dict["num_elems"] = m
            else:
                array_dict["num_elems"] = (key + "_array_length").replace(".", "_").upper();
            ...
            if(msg[key] in primitive_lib.keys()):
                ...
                num_msg_elems += 1
                primitive["name"] = key
                primitive["dtype"] = primitive_lib[msg[key]]
                primitives.append(primitive)
            else:
                log.debug("no match found for {} in primitive_lib".format(msg[key]))
                
            if(msg[key][0:2] == "#_" and not datatype_found):
                log.debug("msg datatype: " + msg[key])
                sorted_dict["datatype"] = msg[key].replace("#_", "")
                datatype_found = True

            if(msg[key][0:3] == "##_" and not includepath_found):
                log.debug("msg include path: " + msg[key])
...
primitive_lib["uint8"] = "uint8_t"
primitive_lib["uint16"] = "uint16_t"
primitive_lib["uint32"] = "uint32_t"
primitive_lib["uint64"] = "uint64_t"
primitive_lib["size_t"] = "uint32_t"

primitive_lib["int8"] = "int8_t"
primitive_lib["int16"] = "int16_t"
primitive_lib["int32"] = "int32_t"
primitive_lib["int64"] = "int64_t"

primitive_lib["float32"] = "float"
primitive_lib["float64"] = "double"

primitive_lib["string"] = "char"
